...
export const InPersonCard = ({
  ...
  const { updateLike, updateSignUp, updateCompleted, updateClicked, updateFeedback } =
    useInteractions();
  ...
  const isPastEvent = cardInfo.end_date
    ? new Date(cardInfo.end_date).getTime() < Date.now()
    : false;

  const shouldShowCompletionPrompt =
    !isAdmin && expanded && clicked && signed_up === true && completed === null && isPastEvent;

  const shouldShowPrompt = !isAdmin && expanded && clicked && signed_up === null;
  ...
  const handleSignUp = async (response: boolean) => {
    ...
    // Update context
    updateSignUp(
      { cardType: 'in_person', cardInfo, liked, signed_up: response, completed, clicked, feedback },
      response,
    );
    toggleExpanded();
  };
  ...
  const handleCompletion = async (response: boolean) => {
    ...
    await addCompletion('interactions_eco_inperson', cardInfo.id, user.id, 'action_id', response);
  ...
  const handleLikes = async (cardInfo: InPersonCardData) => {
    if (user?.id) {
      await toggleLike('interactions_eco_inperson', cardInfo.id, user.id, liked, 'action_id');
      updateLike(
        {
          cardType: 'in_person',
          cardInfo,
              ...
              <View style={{ position: 'relative' }}>
                ...
                {!isAdmin && (completed || signed_up) && (
                  <View style={CardStyles.userTag}>
                    ...
                    <Text style={CardStyles.userTagText}>
                      {completed ? 'Completed' : 'Signed Up'}
                    </Text>
                  </View>
                )}
              </View>
            ...
            {cardInfo.sign_up_link && (
              <View style={CardStyles.signUpButtonContainer}>
                <Pressable
                  ...
                  onPress={() => openSignUpLink(cardInfo.sign_up_link!)}
                >
                  {signed_up ? (
